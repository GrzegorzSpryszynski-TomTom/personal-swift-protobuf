...
struct ConflictingImportMessage:
    GSSwiftProtobuf.Message,
    GSSwiftProtobuf._MessageImplementationBase,
    GSSwiftProtobuf._ProtoNameProviding {
  static let protoMessageName: String = "protobuf_unittest_import.ImportMessage"

  var unknownFields = GSSwiftProtobuf.UnknownStorage()

  init() {}

  mutating func decodeMessage<D: GSSwiftProtobuf.Decoder>(decoder: inout D) throws {
    while let _ = try decoder.nextFieldNumber() {
    }
  }

  func traverse<V: GSSwiftProtobuf.Visitor>(visitor: inout V) throws {
    try unknownFields.traverse(visitor: &visitor)
  }

  static let _protobuf_nameMap: GSSwiftProtobuf._NameMap = [:]
